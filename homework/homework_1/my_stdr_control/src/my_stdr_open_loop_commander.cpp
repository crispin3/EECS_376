...
int main(int argc, char **argv) {
    ...
    double neg_yaw_rate = -0.5; // -0.5 rad/sec yaw rate command - added by Jonathan Crispin
    ...
    // The following code modified by Jonathan Crispin
    twist_cmd.linear.x=0.0; //stop moving forward
    twist_cmd.angular.z=neg_yaw_rate; //and start spinning in place
    timer=0.0; //reset the timer
    while(timer<time_3_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
    ...
    //halt the motion and end of modifications by Jonathan Crispin
    twist_cmd.angular.z=0.0; 
    twist_cmd.linear.x=0.0; 
    for (int i=0;i<10;i++) {
      twist_commander.publish(twist_cmd);
