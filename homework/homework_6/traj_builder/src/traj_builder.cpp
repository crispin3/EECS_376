...
// Modified by Jonathan Crispin from this point on
...
// Pose Information - bookeeping
    ...
    double psi_des = atan2(dy, dx); // Might no be necessary
    double dpsi = min_dang(psi_end - psi_start);
    double t_ramp = 0.0;

// Speed Information - bookeeping

    double speed_des = des_state.twist.twist.linear.x; // Linear speed
    double spin_des = des_state.twist.twist.angular.z; // Angular speed
 
// Linear and angular Deceleration
// Robot in either in cruisin speed or accelerating

while (speed_des > path_move_tol_ || spin_des > path_move_tol_) {
    if (speed_des > path_move_tol_) {
        ROS_INFO("Linear braking");
    ...
    else if (spin_des > path_move_tol_) {
	ROS_INFO("Angular braking");
        t_ramp = spin_des / alpha_max_; 
        npts_ramp = round(t_ramp / dt_); 
        des_state.twist.twist = halt_twist_; // insist on starting from rest  	
        x_des += speed_des * dt_ * cos(psi_des); //Euler one-step integration
        y_des += speed_des * dt_ * sin(psi_des); //Euler one-step integration        
        ...
        vec_of_states.push_back(des_state);
	    for (int i = 0; i < npts_ramp; i++) {
		spin_des -= alpha_max_*dt_; //Euler one-step integration
		des_state.twist.twist.angular.z = spin_des;
		psi_des += spin_des*dt_; //Euler one-step integration
		des_state.pose.pose.orientation = convertPlanarPsi2Quaternion(psi_des);
		vec_of_states.push_back(des_state);
	                                        }
                                         }
    else {
        des_state.twist.twist = halt_twist_; // Just in case
        vec_of_states.push_back(des_state); // Just in case
         }
                                                                 }
...
// Modifications by Jonathan ended
